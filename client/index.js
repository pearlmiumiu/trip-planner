...
mapboxgl.accessToken = "pk.eyJ1Ijoic2htdWVsbG90bWFuIiwiYSI6ImNqYWI5bW0zaTEwdjczMnFpaGRidm9yYmIifQ.XkFSrgeqm-UOgvN0YpY7jQ";
...
const buildAttractionAssets = (category, attraction) => {
  ...
  console.log(state.selectedAttractions)
  //ADD TO DOM
...
function loadItinerary() {
  ...
  api.fetchItinerary(location.hash.substring(1, location.hash.length))
  .then((itineraries) => {
     //let {hotels, restaurants, activities} = itineraries;
     ["hotels", "restaurants", "activities"].forEach(attractionType => {
       document.getElementById(`${attractionType}-list`).innerHTML = '';
        itineraries[attractionType].forEach(attraction => {
